...
def main(src_dir):
    # Load main config
    last_component = os.path.basename(src_dir)
    ...
    if len(pathes_to_imgs):
        ...
        final_table.to_csv("result/" + last_component + ".csv", index=False)
...
def get_classes(image_path):
    ...
    # Load imgs from source dir
    pathes_to_imgs = [i for i in Path(image_path).glob("*")
                      if i.suffix.lower() in [".jpeg", ".jpg", ".png"]]
...
if __name__ == '__main__':
    ...
    for folder in all_items:
        main(os.path.join(folder_path, folder))
